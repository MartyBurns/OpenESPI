...
<xsl:template name="process-assert">
   ...
   <xsl:element name="assert">
	   <xsl:variable name="resultStr">
		   <xsl:apply-templates mode="text"/>
	   </xsl:variable>
	   <xsl:attribute name="TestID">
		   <xsl:value-of select="normalize-space(substring-before($resultStr,'|'))"/>
	   </xsl:attribute>			   
		   
	   <xsl:element name="role"><xsl:value-of select="$role"/></xsl:element>
	   
	   <xsl:element name="TestName">
		   <xsl:value-of select="normalize-space(substring-before(substring-after($resultStr,'|'),'|'))"/>
	   </xsl:element>	
	   <xsl:element name="Report">
		   <xsl:value-of select="normalize-space(substring-after(substring-after($resultStr,'|'),'|'))"/>
	   </xsl:element>	
	   
		<xsl:if test="$diagnose = 'yes'">
			<xsl:element name="diagnostics">
