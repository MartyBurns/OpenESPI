...
<!--
==========================================================================
 Stylesheet: GBESPIRender_xslt1.xsl
    Version: 0.7 20120116
			...
			<body>
				<script type="text/javascript">initComp();</script>			
					<xsl:for-each select="$XML">
						<br/>
						<table class="VisibleTable" width="100%">
						...
						<table class="VisibleTable"  width="100%">
							<thead class="TableHeader">
								<tr>
									<th>
										<xsl:text>Detailed Electricity Usage</xsl:text>
										<br/>
										<br/>
										...
										<xsl:for-each select="atom:feed">
											<xsl:for-each select="atom:entry">
												<xsl:for-each select="atom:content">
													<xsl:for-each select="espi:ElectricPowerUsageSummary">
														<span>
															<xsl:value-of select="format-number((espi:statusTimeStamp - espi:billingPeriod/espi:start) div(86400),&apos;#,###,##0.00&apos;)"/>
														</span>
														<span>
...
<script type="text/javascript">compAddValueAndDate('<xsl:for-each select="espi:value"><xsl:apply-templates/></xsl:for-each>','<xsl:for-each select="espi:timePeriod"><xsl:for-each select="espi:start"><xsl:call-template name="Date"/></xsl:for-each></xsl:for-each>','<xsl:for-each select="espi:cost"><xsl:value-of select="format-number( . div(100000),&quot;0.00&quot;)"/></xsl:for-each>');</script>													
														<tr>
															<td>
