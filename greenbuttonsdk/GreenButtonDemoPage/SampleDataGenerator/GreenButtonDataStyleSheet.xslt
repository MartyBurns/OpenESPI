...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:espi="http://naesb.org/espi" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:variable name="gl_PresentationMultiplier" select="3"/>
	<xsl:template match="/">
		<html>
			<head>
				...
				<xsl:for-each select="$XML">
					...
					<table class="GBDataTable" width="100%">
						<thead>
							<tr>
								<th>
									<xsl:text>Detailed Energy Usage</xsl:text>
									<br/>
									<br/>
									<!-- find start date -->
									...
									<xsl:for-each select="/atom:feed/atom:entry[*]/atom:content/espi:IntervalBlock[last()]/espi:interval">
										<span>
...
========================================================================== -->
	...
	<xsl:template name="ScaleValue">
		...
		<xsl:variable name="presentationMultiplier">
			<xsl:choose>
				<xsl:when test="$gl_PresentationMultiplier=0">1</xsl:when>
				<!--  0 = None -->
				<xsl:when test="$gl_PresentationMultiplier=1">0.1</xsl:when>
				<!--  1 = deca=x10 -->
				<xsl:when test="$gl_PresentationMultiplier=2">0.01</xsl:when>
				<!--  2 = hecto=x100 -->
				<xsl:when test="$gl_PresentationMultiplier=3">0.001</xsl:when>
				<!--  3 = kilo=x1000 -->
				<xsl:when test="$gl_PresentationMultiplier=6">0.00001</xsl:when>
				<!--  6 = Mega=x106 -->
				<xsl:when test="$gl_PresentationMultiplier=9">0.00000001</xsl:when>
				<!--  9 = Giga=x109 -->
				<xsl:when test="$gl_PresentationMultiplier=-3">1000</xsl:when>
				<!--  -3 = mili=x10-3 -->
				<xsl:when test="$gl_PresentationMultiplier=-6">1000000</xsl:when>
				<!--  -6 = micro=x10-3 -->
				<xsl:otherwise>1</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
...
========================================================================== -->
		...
		<xsl:choose>
			<xsl:when test=". = 72">
				<xsl:variable name="DisplayUnits">
					<xsl:choose>
						<xsl:when test="$gl_PresentationMultiplier=0"/>
						<!--  0 = None -->
						<xsl:when test="$gl_PresentationMultiplier=1">deca</xsl:when>
						<!--  1 = deca=x10 -->
						<xsl:when test="$gl_PresentationMultiplier=2">hecto</xsl:when>
						<!--  2 = hecto=x100 -->
						<xsl:when test="$gl_PresentationMultiplier=3">kilo</xsl:when>
						<!--  3 = kilo=x1000 -->
						<xsl:when test="$gl_PresentationMultiplier=6">mega</xsl:when>
						<!--  6 = Mega=x106 -->
						<xsl:when test="$gl_PresentationMultiplier=9">giga</xsl:when>
						<!--  9 = Giga=x109 -->
						<xsl:when test="$gl_PresentationMultiplier=-3">milli</xsl:when>
						<!--  -3 = mili=x10-3 -->
						<xsl:when test="$gl_PresentationMultiplier=-6">micro</xsl:when>
						<!--  -6 = micro=x10-3 -->
						<xsl:otherwise>1</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
	...
	<xsl:template name="Date2">
		...
		<xsl:variable name="strTemp">
			<xsl:call-template name="convert-unix-to-date-time">
				<xsl:with-param name="dtTmp" select="$pdtTmp"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:value-of select="concat(substring-before($strTemp , 'T'), ' ', substring-after( $strTemp , 'T'))"/>
		<span>
